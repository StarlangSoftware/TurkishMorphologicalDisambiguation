...
public abstract class AutoDisambiguator {
    ...
    private static boolean isAnyWordSecondPerson(int index, ArrayList<FsmParse> correctParses) {
        for (int i = index - 1; i >= 0; i--) {
            String surfaceForm = correctParses.get(i).getSurfaceForm();
            if ("sen".equals(surfaceForm) || "Sen".equals(surfaceForm)) {
                return true;
            }
        }
        return false;
    ...
    private static boolean isBeforeLastWord(int index, FsmParseList[] fsmParses){
        if (fsmParses == null || fsmParses.length == 0) return false;

        int lastIndex = fsmParses.length - 1;
        String lastSurface = fsmParses[lastIndex].getFsmParse(0).getSurfaceForm();
        if (".".equals(lastSurface) || "?".equals(lastSurface) || "!".equals(lastSurface)) {
            lastIndex--;
        }
    ...
    private static String selectCaseForParseString(String parseString, int index, FsmParseList[] fsmParses, ArrayList<FsmParse> correctParses) {
        ...
        switch (parseString) {
            ...
            case "A2SG+P2SG$A3SG+P3SG":
                ...
                return "A3SG+P3SG";
            /* BİR */
            case "ADJ$ADV$DET$NUM+CARD$NUM+CARD^DB+NOUN+ZERO+A3SG+PNON+NOM":
                return "DET";
            ...
            case "A3PL+PNON+NOM$A3SG+PNON+NOM^DB+VERB+ZERO+PRES+A3PL$PROP+A3PL+PNON+NOM":
                if (isBeforeLastWord(index, fsmParses)) {
                    return "A3SG+PNON+NOM^DB+VERB+ZERO+PRES+A3PL";
                }
                if (index > 0) {
            ...
            /* fazlası, yetkilisi */
            case "ADJ+JUSTLIKE$ADJ+JUSTLIKE^DB+NOUN+ZERO+A3SG+PNON+NOM$NOUN+ZERO+A3SG+P3SG+NOM":
            ...
            case "ADJ+PASTPART+P1PL$NOUN+PASTPART+A3SG+P1PL+NOM$NOUN+PASTPART+A3SG+P1SG+NOM^DB+VERB+ZERO+PRES+A1PL":
            ...
            case "NOUN+PROP+A3SG+PNON+NOM$VERB+POS+IMP+A2SG":
                return "NOUN+PROP+A3SG+PNON+NOM";
            case "ADJ+JUSTLIKE$ADJ+JUSTLIKE^DB+NOUN+ZERO+A3SG+PNON+NOM$NOUN+ZERO+A3SG+P3SG+NOM$NOUN+ZERO^DB+ADJ+ALMOST":
                return "NOUN+ZERO+A3SG+P3SG+NOM";
            ...
            case "NOUN+A3SG+PNON+NOM$NUM+CARD$VERB+POS+IMP+A2SG":
                if (isNextWordNum(index, fsmParses) || isNextWordNounOrAdjective(index, fsmParses)) {
                    return "NUM+CARD";
                }
                return "NOUN+A3SG+PNON+NOM";
            ...
            case "ADJ$ADV$POSTP+PCINS":
                ...
                return "ADV";
            /* BİN, KIRK */
            case "NUM+CARD$NUM+CARD^DB+NOUN+ZERO+A3SG+PNON+NOM$VERB+POS+IMP+A2SG":
                return "NUM+CARD";
            case "CARD$CARD^DB+NOUN+ZERO+A3SG+PNON+NOM":
                return "NUM+CARD";
            case "P2SG+LOC^DB+ADJ+REL$P2SG+LOC^DB+ADJ+REL^DB+NOUN+ZERO+A3SG+PNON+NOM$P3SG+LOC^DB+ADJ+REL$P3SG+LOC^DB+ADJ+REL^DB+NOUN+ZERO+A3SG+PNON+NOM":
                return "NOUN+A3SG+P3SG+LOC^DB+ADJ+REL";
            /* ÖTE */
